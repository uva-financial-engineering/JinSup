...
public abstract class Agent {
  ...
  /**
   * Is true if an order in the last millisecond resulted in a trade with one
   * of the agent's order. False otherwise.
   */
   ...
   * pass in the MatchingEngine that is used for the simulation. Time is set
   * to a negative value so that the agent is not automatically picked to act.
   * The agent is also added automatically to the MatchingEngine's agentMap so 
   * that it can keep track of the agent.
   * @param matchEng
   */
  ...
  /**
   * Method to be implemented by subclasses of Agent. Holds different
   * trading strategy code depending on the type of agent desired.
   */
  abstract void act();
  ...
  /**
   * @return True if the agent still needs to act during its turn. False if 
   * it is done for its turn.
   */
  ...
  /**
   * Enable the agent to act during its time slot. This is set by the 
   * controller when it chooses agents to act for certain time slots.
   * @param If true, then the agent will act.
   * 
   */
   ...
   * @param The order that needs to be cancelled.
   * 
   */
   ...
   * @param price Price of the order (should be in 0.25 increments only).
   * @param initialQuant The quantity that the agent wants to buy/sell.
   * @param buyOrder True if the agent wants to buy. False otherwise.
   */
   ...
   * @param o The order to be modified.
   * @param newPrice The new price to set the order to.
   * @param newQuant The new quantity to set the order to.
   */
  ...
  /**
   * Informs the agent whether an order resulted in a trade or not. This is 
   * set from the MatchingEngine.
   * @param traded True if a trade happened. False otherwise.
   * @param volume Volume that was traded
   */
